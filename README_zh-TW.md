...
| 🛡️ **SUSFS** | **v2.3.0**（`android13-5.10` 為 `f2878eb5`）+ 使用者空間模組 | [SUSFS-FOR-KERNELSU](https://github.com/sidex15/susfs4ksu-module) |
...
- `gki-android13-5.10` 的 susfs4ksu 釘 `f2878eb5`（SUSFS v2.3.0）。其他 GKI 版本在這條分支沒有預設 SUSFS pin。不再套用第三方 KSU 共存 patch。
...
- `susfs_commit_android13-5-10` 留空時用 `f2878eb5`。其他 `susfs_commit_*` 必須自填，沒有 `ee023e3` fallback。
